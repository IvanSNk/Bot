...
async def handle_start(message: types.Message):
    await message.answer(text=f"Здравствуйте, {message.from_user.first_name}! Укажите группу, информацию по которой нужно найти")

@dp.message(Command("list"))
async def handle_help(message: types.Message):
    text = (f"Вот список групп, по которым можно получить информацию: {convert_series_to_string(groups_unique)}")
...
async def choice(message: types.Message):
    if message.text in groups.values:
        ...
        f"В датасете находятся оценки {unique_student_count.size} студентов со следующими личными номерами {message.text}: {convert_series_to_string(unique_student_count)}\n"
        ...
        text = (f"Вы указали группу {message.text}, верно?")
